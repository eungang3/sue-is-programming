...
- Migration : Model을 바탕으로 SQL query를 생성하여 데이터베이스를 업데이트하는 것
...
## 2. Django Models
 - model : 데이터베이스의 스키마를 정의하는 Django 클래스.  
 - 프로젝트 생성 -> 앱 생성 -> 앱 내 models.py에 사용할 모델 입력 (python class 사용)
 - model 하나당 데이터베이스의 main table 하나라고 생각하면 됨
 ...
 ## 3. Migration
- migration : Model을 바탕으로 SQL query를 생성하여 데이터베이스를 업데이트하는 것. DB 스키마를 git처럼 버전 관리할 수 있음
- 모델 바탕으로 migration 만들기 ```python manage.py makemigrations``` -> 앱 내 migration 디렉토리 생성, 디렉토리 내 init 파일과 0001_initial.py 파일 생성됨. model에 변경이 일어나면 0001_initial.py 파일에 반영됨
...
# <QuerySet [<Flight: flight object(1)>]> 뜸
{% endhighlight %}

- 출력되는 object를 읽기 쉬운 string으로 바꿔주는 ```__str___``` method 만들기
- 
{% python highlight %}
# models.py
...
class Project(models.Model):
   ...
   duration = models.IntegerField()
   def __str__(self):
    return f"{self.id}: title is {self.title}. Duration is {self.duration}"
{% endhighlight %}
