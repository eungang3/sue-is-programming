...
### 1) Many to One relationship
 - A 테이블의 id가 B 테이블의 여러 id와 연결될 수 있지만, B 테이블은 A 테이블의 한 id만 가질 수 있는 경우
 - ForeignKey 필드 사용
  
 {% highlight python %}
 # 한 기사는 한 저자만 갖지만, 한 저자가 여러 기사를 쓸 수는 있음
 # models.py
 ...
 class Writer(models.Model):
  ...
  class Article(models.Model):
    ...
    author = models.ForeignKey(Writer, on_delete=models.CASCADE, related_name="authors")
...
class Writer(models.Model):
    name = models.CharField(max_length=64)
    books = models.ManyToManyField(Book, blank=True, related_name="writers")
